...
export const BlocklyWorkspace = forwardRef(function BlocklyWorkspace({ onChange }, ref) {
	...
	const [toolboxVisibleFn] = useState(() => (ws, visible) => {
	...
	useImperativeHandle(
		...
		() => ({
			...
			setToolboxOpen(open) {
				const ws = workspaceRef.current;
				if (!ws) return;
				toolboxVisibleFn(ws, open);
				setToolboxOpen(open);
			},
		}),
		[toolboxOpen, toolboxVisibleFn],
	);
	...
	return (
		<div className="relative size-full" style={{ minHeight: 0 }}>
			<div ref={containerRef} className="size-full" style={{ minHeight: 0 }} />
			<div
