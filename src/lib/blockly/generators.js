...
export function registerGenerators() {
	...
	javascriptGenerator.forBlock.tobi_when_start = () => "";

	// ── Movimiento ──
	javascriptGenerator.forBlock.tobi_walk = (block) => {
		const steps = block.getFieldValue("STEPS");
		return `caminar(${steps});`;
	};

	javascriptGenerator.forBlock.tobi_walk_backward = (block) => {
		const steps = block.getFieldValue("STEPS");
		return `retroceder(${steps});`;
	};

	javascriptGenerator.forBlock.tobi_rotate = (block) => {
		const angle = block.getFieldValue("ANGLE");
		return `girar(${angle});`;
	};

	javascriptGenerator.forBlock.tobi_turn_left = (block) => {
		const angle = block.getFieldValue("ANGLE");
		return `girar(${-angle});`;
	};

	javascriptGenerator.forBlock.tobi_turn_right = (block) => {
		const angle = block.getFieldValue("ANGLE");
		return `girar(${angle});`;
	};

	javascriptGenerator.forBlock.tobi_speed = (block) => {
		const percent = block.getFieldValue("PERCENT");
		return `velocidad(${percent});`;
	};

	javascriptGenerator.forBlock.tobi_jump = () => "saltar();";

	javascriptGenerator.forBlock.tobi_incline = (block) => {
		const angle = block.getFieldValue("ANGLE");
		return `inclinarse(${angle});`;
	};

	javascriptGenerator.forBlock.tobi_tail_wag = () => "moverCola();";

	javascriptGenerator.forBlock.tobi_sit = () => "sentarse();";

	javascriptGenerator.forBlock.tobi_stand = () => "pararse();";

	javascriptGenerator.forBlock.tobi_dance = () => "bailar();";

	javascriptGenerator.forBlock.tobi_stop = () => "detener();";

	// ── Control ──
	javascriptGenerator.forBlock.tobi_wait = (block) => {
		const seconds = block.getFieldValue("SECONDS");
		return `esperar(${seconds});`;
	};

	javascriptGenerator.forBlock.tobi_repeat = (block) => {
	...
	javascriptGenerator.forBlock.tobi_for = (block) => {
	...
	javascriptGenerator.forBlock.tobi_while = (block) => {
		const condition =
			javascriptGenerator.valueToCode(block, "CONDITION", Order.CONDITIONAL) || "true";
		const branch = javascriptGenerator.statementToCode(block, "DO");
	...
	javascriptGenerator.forBlock.logic_compare = (block) => {
	...
	javascriptGenerator.forBlock.logic_operation = (block) => {
	...
	// ── Bucles (nativos) ──
	javascriptGenerator.forBlock.controls_repeat_ext = (block) => {
	...
	javascriptGenerator.forBlock.controls_whileUntil = (block) => {
	...
	// ── Matemáticas ──
	javascriptGenerator.forBlock.math_number = (block) => {
	...
	javascriptGenerator.forBlock.math_arithmetic = (block) => {
		...
		const ops = {
			ADD: ["+", Order.ADD],
			MINUS: ["-", Order.SUBTRACT],
			MULTIPLY: ["*", Order.MULTIPLY],
			DIVIDE: ["/", Order.DIVIDE],
			POWER: ["^", Order.EXPONENTIATION],
		};
		const [sym, order] = ops[op] || ["+", Order.ADD];
		return op === "POWER" ? [`pow(${a}, ${b})`, order] : [`${a} ${sym} ${b}`, order];
	};

	javascriptGenerator.forBlock.math_single = (block) => {
